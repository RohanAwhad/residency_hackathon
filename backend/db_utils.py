...
@with_connection
def insert_paper_n_references(
    conn, paper: data_models.Papers, references: list[data_models.References]
) -> None:
    insert_papers_query = sql.SQL(
        """INSERT INTO papers (paper_url, title, authors, abstract, sections_text, sections_json) VALUES (%s, %s, %s, %s, %s, %s)"""
    )
    paper_item = (
        str(paper.paper_url),
        paper.title,
    ...
    reference_items = [
        (
            ref.referred_by_paper_url,
            ref.reference_id,
            ref.referred_sections,
            ref.title,
            ref.authors,
            ref.journal,
            ref.year,
...
@with_connection
def insert_paper_ref_embeddings(
    conn,
    paper: data_models.Papers,
    references: list[data_models.References],
    embeddings: list[data_models.EmbeddingsIn],
) -> None:
    # papers query
    insert_papers_query = sql.SQL(
        """INSERT INTO papers (paper_url, title, authors, abstract, sections_text, sections_json) VALUES (%s, %s, %s, %s, %s, %s)"""
    ...
    # embeddings query
    insert_embeddings_query = sql.SQL(
        """INSERT INTO embeddings_table (paper_url, chunk, embedding) VALUES (%s, %s, %s)"""
    )
...
def save_mindmap(conn, mindmap: str, paper_url: str):
    update_query = sql.SQL(
        """UPDATE papers SET summary_markdown = %s WHERE paper_url = %s"""
    )
...
def search_paper_by_title(conn, query_title: str) -> Optional[data_models.SearchResult]:
    search_query = sql.SQL(
        "SELECT paper_url, title, authors FROM papers WHERE fts @@ TO_TSQUERY(%s)"
    )
    ...
    if result:
        return data_models.SearchResult(*result)
    return None
...
def insert_batch_references(conn, references: list[data_models.References]) -> None:
    ...
    with conn.cursor() as cur:
        cur.executemany(sql.SQL(insert_query), items)


@with_connection
def insert_reference_info(
    conn,
    referred_by_url: str,
    ref_id: str,
    ref_url: str,
...
@with_connection
def get_reference_info_for_searching(
    conn, referred_by_paper_url, reference_id: str
) -> data_models.RefInfoOut:
    read_query = sql.SQL(
        """
  ...
  """
    ...
    return data_models.RefInfoOut(
        title=res[0],
        authors=(
            [x.strip() for x in res[1].split("; ") if x.strip()] if res[1] else None
        ),
...
@with_connection
def get_processed_reference(
    conn, paper_url: str, ref_id: str
  ...
  """
    ...
    return data_models.ProcessRefOut(
        ref_url, title, author, q1_ans, q2_ans, q3_ans, deleted=False
    )
...
def get_reference_urls(conn, paper_url: str) -> Optional[list[str]]:
    query = sql.SQL(
        """SELECT referred_paper_url FROM references_table WHERE referred_by_paper_url = %s;"""
    )
    item = (paper_url,)
    with conn.cursor() as cur:
...
def remove_reference(conn, paper_url: str, ref_id: str):
    ...
    delete_query = sql.SQL(
        """DELETE FROM references_table WHERE referred_by_paper_url = %s AND reference_id = %s"""
    )
    item = (paper_url, ref_id)
    with conn.cursor() as cur:
...
def get_top_k_similar(
    conn, query_embedding: list[float], paper_urls: list[str], k: int
) -> list[data_models.EmbeddingsOut]:
    search_query = sql.SQL(
        """
  ...
  """
    ...
    return [
        data_models.EmbeddingsOut(paper_url=res[0], chunk=res[1], sim_score=res[2])
        for res in results
    ]
