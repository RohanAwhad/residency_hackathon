...
async def process_reference(paper_url: str, ref_id: str):
    ...
    if ret:
        return ret
    raise HTTPException(
        status_code=500, detail="Couldn't generate information about this reference"
    )
...
async def generate_chat_response(inp: ChatReqIn):
    ...
    if query_embeddings is None:
        raise HTTPException(
            status_code=500, detail="Couldn't generate embeddings from user message"
        )
    ...
    # get papers content
    main_paper = db_utils.read_paper(paper_url).sections_text
    if main_paper is None:
        raise HTTPException(status_code=500, detail="Couldn't find paper's text in DB")

    # get context from references
    ...
    if context is None:
        print("Couldn't retrieve context")
    response = utils.generate_chat_response(context, inp.messages, main_paper)
    return response
...
async def stream_response(
    iter, callable
):  # callable is the function that will be called after all chunks are sent to save it in DB
    if mindmap_chunks:
...
async def read_minmap_md(url: str) -> str:
    ...
    if paper is None:
        raise HTTPException(status_code=500, detail="Couldn't find paper in DB")
    print("Mindmap in DB:", paper.mindmap)
    if paper.mindmap is None:
        mindmap = utils.generate_mindmap(paper)
        if mindmap is None:
            raise HTTPException(
                status_code=500, detail="Couldn't generate mindmap of the paper"
            )
...
async def generate_code(inp: CodeReqIn):
    if not inp.paper_url.endswith(".pdf"):
        inp.paper_url += ".pdf"
    ...
    if paper.code is None:
        ...
        if code is None:
            raise HTTPException(
                status_code=500, detail="Couldn't generate code for the paper"
            )
